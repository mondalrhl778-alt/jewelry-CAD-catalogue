...
document.getElementById('renderFiles').addEventListener('change',async event=>{
  const files=[...event.target.files];
  if(!files.length)return;
  const images={};
  for(const file of files){
    const key=file.name.toLowerCase().replace(/\.png$/,'');
    if(['hero','front','side','back','top'].includes(key)) images[key]=await readImage(file);
  }
  if(images.hero){document.getElementById('heroRender').src=images.hero;document.getElementById('productArea').classList.add('has-render');}
  for(const view of ['front','side','back']){
    const source=images[view]||(view==='back'?images.top:null);
    if(source){document.getElementById(`${view}Render`).src=source;document.getElementById(`${view}View`).classList.add('has-render');}
  }
